...
def annuity_payment(amount, interest_rate, term):
    ann_paym = round((amount * (interest_rate / 12)) / (1 - (1 + (interest_rate / 12)) ** (-term)), 2)
    return ann_paym
...
def start():
    def calculate():
        ...
        try:
            loan_amount = float(loan_amount_getter.get())  # изначальная сумма кредита
        except ValueError:
        ...
        try:
            interest_rate = float(interest_rate_getter.get()) / 100  # размер процентной ставки
        except ValueError:
            incorrect_value = ttk.Label(master=main_frame, text='Неверные данные')
            incorrect_value.grid(row=1, column=2)
            main_window.mainloop()

        try:
            term = int(term_getter.get())  # срок кредита
        except ValueError:
            incorrect_value = ttk.Label(master=main_frame, text='Неверные данные')
            incorrect_value.grid(row=2, column=2)
            main_window.mainloop()
        ...
        with open('Repayment_schedule.txt', 'w', encoding='utf-8') as file:
            ...
            ready_button = ttk.Button(master=main_frame, text='Сформировать заново', command=start)
